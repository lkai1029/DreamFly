 ...
 * @description:
 *               <p>
 *               给定两个大小为 m 和 n 的有序数组 nums1 和 nums2 。 请找出这两个有序数组的中位数。要求算法的时间复杂度为
 *               O(log (m+n)) 。 PS:中位数--如果是奇数个，是中间的那个；如果是偶数个，是中间两个的平均数。 你可以假设
 *               nums1 和 nums2 不同时为空。
 *               </p>
 *               示例1： nums1 = [1, 2] nums2 = [3, 4] 中位数是 (2 + 3)/2 = 2.5 示例2：
...
public class Median {
	...
	/**
	 * 时间复杂度：O(log(min(m,n)))， 首先，查找的区间是 [0, m]。 而该区间的长度在每次循环之后都会减少为原来的一半。
	 * 所以，我们只需要执行 log(m) 次循环。由于我们在每次循环中进行常量次数的操作，所以时间复杂度为 O(log(m))。 由于 m <=
	 * n，所以时间复杂度是 O(log(min(m,n)))。
	 * <p>
	...
	public double findMedianSortedArrays(int[] A, int[] B) {
		/**
		 * 以i作为A的切割索引，j作为B的切割索引，i和j的左边组合，与右边组合满足中位数的要求 i = m - i; j = n - j 左右个数相同 => i
		 * + j = m - i + n - j + 1; // 为何加1？i，j如果是奇数长度的数组的中位数，包含1/2的可能 => 2i + 2j = m +
		 * n + 1; => j = (m + n + 1) / 2 - i; // 必要条件
		 *
		 * halfLen : 整合为一个数组的中位数索引 n必须大于m长度：如果m大于n，可能会出现m的一半i大于(m + n + 1)/2 的情况，使j小于0
		 */
		int m = A.length;
		int n = B.length;
		// to ensure m<=n
		...
		/**
		 * 完成条件：i和j的左边是整体中位数以左的部分 B[j - 1] <= A[i] && A[i - 1] <= B[j]
		 */
		while (iMin <= iMax) {
			System.out.println("===============");
